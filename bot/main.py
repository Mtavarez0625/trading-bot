...
from exit_manager import monitor_and_manage_exits
...
def run_cycle(
    ...
    # Check open positions FIRST — log snapshots and apply exit logic before
    # evaluating any new entries.
    monitor_and_manage_exits(trading_client)

    tradable, alpaca_equity = is_account_tradable(trading_client)
    if not tradable:
    ...
    if config.dry_run:
        ...
        log.info(
            "[DRY RUN] Sizing equity: $%.2f (simulated PAPER_ACCOUNT_EQUITY — "
            "Alpaca account equity $%.2f is ignored for sizing)",
    ...
    # Market regime filter — skip all new long entries if SPY is not bullish.
    # Fail-safe: if SPY data is unavailable, check_market_regime returns bearish.
    regime = check_market_regime(data_client, timeframe)
    if not regime.is_bullish:
        log.warning(
            "Market regime: NOT BULLISH — no new entries this cycle. Reason: %s",
            regime.reason,
        )
        return
    log.info("Market regime: BULLISH — %s", regime.reason)

    # Fetch open position count once — updated locally as trades execute
    open_count = count_open_positions(trading_client)
    trade_syms = config.trade_watchlist()
    log.info(
        "Open positions: %d/%d | Trade symbols: %s | Regime ETFs: %s",
        open_count, config.max_positions, trade_syms, list(config.index_etfs),
    )

    # Mutable ref so _evaluate_symbol can update it after each execution
    open_count_ref = [open_count]

    for symbol in trade_syms:
...
def _evaluate_symbol(
    ...
    # Guard: correlated ETF group — only one position from the index-ETF group at a time.
    # SPY and QQQ are treated as a single correlated group (both track the broad US market).
    if config.is_in_etf_group(symbol) and config.index_etfs:
        group_count = count_group_positions(trading_client, config.index_etfs)
        if group_count >= config.max_etf_group_positions:
            log.info(
                "[%s] Correlated position already exists. Skipping.",
                symbol,
            )
            tracker.record_skip(symbol, "correlated_position_exists")
            return
    ...
    max_alloc_usd  = equity * config.max_allocation_pct
    raw_risk_qty   = int(math.floor(risk_amount / stop_distance)) if stop_distance > 0 else 0
    max_alloc_qty  = int(math.floor(max_alloc_usd / entry_price)) if entry_price > 0 else 0

    qty = compute_quantity(
        equity, config.risk_per_trade, entry_price,
        config.stop_loss_pct, config.max_allocation_pct,
    )

    log.info(
        "[%s] Sizing | price=%.2f | equity=%.2f | "
        "max_alloc=$%.2f (%.0f%%) | risk_$=%.2f (%.1f%%) | "
        "stop_pct=%.1f%% | risk_qty=%d | alloc_qty=%d | final_qty=%d",
        symbol, entry_price, equity,
        max_alloc_usd, config.max_allocation_pct * 100,
        risk_amount, config.risk_per_trade * 100,
        config.stop_loss_pct * 100,
        raw_risk_qty, max_alloc_qty, qty,
    )

    if qty < 1:
        log.warning(
            "[%s] SKIP: qty=0 — $%.2f/share unaffordable "
            "(equity=$%.2f, max_alloc=$%.2f at %.0f%%). "
            "This is a SKIP, not an entry.",
            symbol, entry_price, equity, max_alloc_usd,
            config.max_allocation_pct * 100,
        )
    ...
    # Extract order details for the trade log
    order_id = str(getattr(order, "id", "")) if order else ""
    order_status = str(getattr(order, "status", "")) if order else (
        "dry_run" if config.dry_run else "unknown"
    )
...
def main() -> None:
    ...
    if not config.dry_run:
        log.warning(
            "*** DRY_RUN is FALSE — real paper orders WILL be submitted to Alpaca. ***"
            " Set DRY_RUN=true to evaluate signals without placing orders."
        )
    else:
        log.info("--- DRY RUN CONFIG SUMMARY ---")
        log.info(
            "Simulated equity : $%.2f  (PAPER_ACCOUNT_EQUITY)",
            config.paper_account_equity,
        )
        log.info(
            "Max position val : $%.2f  per trade  (MAX_ALLOCATION_PCT=%.0f%%)",
            config.paper_account_equity * config.max_allocation_pct,
            config.max_allocation_pct * 100,
        )
        log.info(
            "Risk amount      : $%.2f  per trade  (RISK_PER_TRADE=%.1f%%)",
            config.paper_account_equity * config.risk_per_trade,
    ...
    log.info(
        "Max ETF group: %d  (correlated ETF group: %s)",
        config.max_etf_group_positions, list(config.index_etfs) or "(none)",
    )
    log.info(
        "Max allocation: %.1f%%  (max single-position value vs equity)",
        config.max_allocation_pct * 100,
    )
    log.info("=" * 60)
    ...
    if account is not None:
        try:
            ...
            log.info(
                ...
                alpaca_equity, buying_power, status,
            )
            if config.dry_run:
                session_start_equity = config.paper_account_equity
                log.info(
                    "[DRY RUN] Sizing uses simulated equity: $%.2f (PAPER_ACCOUNT_EQUITY). "
                    "Alpaca buying power is ignored.",
                    session_start_equity,
                )
            else:
                session_start_equity = alpaca_equity
        ...
        if config.dry_run:
            session_start_equity = config.paper_account_equity
            log.warning(
                "Could not fetch Alpaca account snapshot. "
                "[DRY RUN] Falling back to simulated equity: $%.2f",
                session_start_equity,
            )
            state.set_start_equity(session_start_equity)
        else:
    ...
    try:
        while True:
            ...
            if now_et >= market_close:
                log.info("Market close fail-safe triggered. Shutting down bot.")
                break
